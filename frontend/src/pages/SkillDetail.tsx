    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              {data.top_companies.slice(0, 8).map((company, index) => (
                <div
                  ...
                  onClick={() => navigate(`/companies/${encodeURIComponent(company.name.toLowerCase())}`)}
                >
        ...
        <GlassCard>
          <div className="p-6">
            <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-4">
              Skills Often Paired With {data.name}
            </h2>
            <div className="space-y-3">
              {data.related_skills.slice(0, 5).map((skill, index) => (
                <div
                  key={skill.skill}
                  className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 cursor-pointer transition-colors"
                  onClick={() => navigate(`/skills/${skill.skill.toLowerCase().replace(/\s+/g, '-').replace(/\./g, '')}`)}
                >
                  <div className="flex items-center gap-3">
                    <span className="text-sm font-medium text-gray-400 w-6">
                      {index + 1}
                  ...
                  <div className="text-right">
                    <div className="text-sm font-medium text-gray-900 dark:text-gray-100">
                      {skill.co_occurrence_percentage.toFixed(1)}%
                    </div>
            ...
            <div className="mt-4 text-xs text-gray-500 dark:text-gray-400">
              Percentage shows how often this skill appears with {data.name}
            </div>
          </div>
        </GlassCard>
      </div>
      ...
      <GlassCard>
        <div className="p-6">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button
              onClick={() => navigate(`/skills?category=${data.category}`)}
